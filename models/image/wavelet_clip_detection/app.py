    ...
    with model_lock:
        ...
        try:
            ...
            if os.path.exists(actual_model_path):
                ...
                logger.warning(
                    f"Model weights not found at {actual_model_path}. Using uninitialized model."
                )
...
def preprocess_image(image_bytes):
    ...
    try:
        ...
        try:
            ...
            image_tensor = (
                torch.from_numpy(np_img).float().unsqueeze(0)
            )  # Add batch dimension
...
async def predict(image_input: ImageInput):
    ...
    try:
        ...
        logger.info(
            f"Prediction completed in {inference_time:.4f}s. Prob Fake: {probability:.4f}"
        )
