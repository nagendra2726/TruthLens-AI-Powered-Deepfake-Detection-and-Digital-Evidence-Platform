...
class APIClient:
    def __init__(
        self, config_manager, media_type: Optional[str], run_from_host: bool = True
    ):
        self.config_manager = config_manager
        self.media_type = media_type
        self.api_url_config = (
            self.config_manager.get_api_url()
        ...
        if self.run_from_host:
            ...
            if (
                ...
                and parsed_api_url.hostname != "localhost"
                and parsed_api_url.hostname != "127.0.0.1"
            ):
                ...
                self.api_url = self.api_url_config.replace(
                    f"http://{parsed_api_url.hostname}", "http://localhost"
                )
            else:
        ...
        if self.media_type:
            ...
            health_endpoints_config = self.config_manager.get_health_endpoints(
                media_type
            )
            ...
            if self.run_from_host:
                ...
                for name, url_str in model_endpoints_config.items():
                    try:
                        ...
                        if (
                            parsed_url.hostname
                            and parsed_url.hostname != "localhost"
                            and parsed_url.hostname != "127.0.0.1"
                        ):
                            self.model_endpoints[name] = url_str.replace(
                                f"http://{parsed_url.hostname}", "http://localhost"
                            )
                        else:
                            self.model_endpoints[name] = url_str
                    except Exception as e:
                        console.print(
                            f"[yellow]Warning: Could not parse or transform URL '{url_str}' for model '{name}': {e}[/yellow]"
                        )
                        self.model_endpoints[name] = (
                            url_str  # Keep original if parsing/replacement fails
                        )
                ...
                for name, url_str in health_endpoints_config.items():
                    try:
                        ...
                        if (
                            ...
                            self.health_endpoints[name] = url_str.replace(
                                f"http://{parsed_url.hostname}", "http://localhost"
                            )
                        else:
                        ...
                        console.print(
                            f"[yellow]Warning: Could not parse or transform URL '{url_str}' for health '{name}': {e}[/yellow]"
                        )
                        self.health_endpoints[name] = url_str
        ...
        self.timeout = self.config_manager.get_default(
    ...
    def _make_request(
        ...
        try:
            ...
            console.print(
                f"[yellow]Request to {url} timed out (attempt {attempt_num+1}/{self.max_retries+1}).[/yellow]"
            )
            if attempt_num < self.max_retries:
                time.sleep(
                    1 * (attempt_num + 1)
                )  # Simple linear backoff for test client
                return self._make_request(
                    url, method, json_payload, files, data, params, attempt_num + 1
                )
            raise
        ...
        ) as e:  # Catches ConnectionError, NameResolutionError etc.
    ...
    def check_main_api_health(
        self, force_refresh=False
    ) -> Dict[str, Any]:  # force_refresh not used here, config reloaded on init
        ...
        try:
            ...
            console.print(
                f"[bold red]Error checking main API health at {health_url}: {e}[/bold red]"
            )
    ...
    def check_model_health(self, model_name: str) -> Dict[str, Any]:
        if not self.media_type:
            return {
                "status": "error",
                "detail": "Media type not set for APIClient to check model health.",
            }
        if model_name not in self.health_endpoints:
            return {
                "status": "error",
                "detail": f"Unknown model or health endpoint not configured: {model_name} for media type {self.media_type}",
            }
        ...
        try:
            ...
            console.print(
                f"[bold red]Error checking model health for {model_name} at {health_url}: {e}[/bold red]"
            )
            return {"status": "error", "detail": str(e), "url_attempted": health_url}
    ...
    def test_with_main_api(
        ...
        try:
            ...
            console.print(
                f"[bold red]Error testing with main API ({predict_url}): {e}[/bold red]"
            )
            return {
                "error": str(e),
                "media_path": media_path,
                "media_name": os.path.basename(media_path),
                "url_attempted": predict_url,
            }

    def test_with_individual_model(
        self, model_name: str, media_path: str, encoded_media: str, threshold: float
    ) -> Dict[str, Any]:
        if not self.media_type:
            return {
                "error": "Media type not set for APIClient to test individual model."
            }

        if model_name not in self.model_endpoints:
            original_model_endpoints = self.config_manager.get_model_endpoints(
                self.media_type
            )
            if model_name not in original_model_endpoints:
                return {
                    "error": f"Unknown model: {model_name} for media type {self.media_type}. Not found in config.",
                    "model_name": model_name,
                    "media_path": media_path,
                }
            else:
                return {
                    "error": f"Could not determine host URL for model endpoint: {model_name}. Original URL: {original_model_endpoints[model_name]}",
                    "model_name": model_name,
                    "media_path": media_path,
                }
        ...
        if not payload_key:
            return {
                "error": f"Unsupported media_type '{self.media_type}' for individual model {model_name} payload"
            }
        payload[payload_key] = encoded_media
        ...
        try:
            response = self._make_request(
                model_predict_url, "POST", json_payload=payload
            )
            result = response.json()
            ...
            return {
                ...
                "media_path": media_path,
                "media_name": os.path.basename(media_path),
                "url_attempted": model_predict_url,
            }
        finally:
            gc.collect()
    ...
    def request_model_unload(self, model_name: str) -> bool:
        if not self.media_type:
            console.print(
                f"[yellow]Media type not set. Cannot determine health endpoint for {model_name}.[/yellow]"
            )
            return False
        if model_name not in self.health_endpoints:
            original_health_endpoints = self.config_manager.get_health_endpoints(
                self.media_type
            )
            if model_name not in original_health_endpoints:
                console.print(
                    f"[yellow]No health endpoint defined for {model_name} (type: {self.media_type}), cannot send unload request.[/yellow]"
                )
            else:
                console.print(
                    f"[yellow]Could not determine host URL for {model_name} health endpoint. Original URL: {original_health_endpoints[model_name]}[/yellow]"
                )
            return False
        ...
        try:
            console.print(
                f"[cyan]Requesting unload for model {model_name} at {unload_url}...[/cyan]"
            )
            response = requests.post(unload_url, timeout=20)
            if response.status_code == 200:
                console.print(
                    f"[green]Successfully requested unload of model {model_name}. Response: {response.text[:100]}[/green]"
                )
                return True
            else:
                console.print(
                    f"[yellow]Unload request for {model_name} failed or not supported. Status: {response.status_code}, Text: {response.text[:100]}[/yellow]"
                )
                return False
        except requests.exceptions.ConnectionError:
            console.print(
                f"[yellow]Could not connect to {unload_url} for {model_name}. Model service might not have /unload or is down.[/yellow]"
            )
            return False
        except Exception as e:
            console.print(
                f"[red]Error during unload request for {model_name}: {e}[/red]"
            )
            return False
